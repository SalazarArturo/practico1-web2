import{ getAllCanchas, createCancha, getCanchaById, updateCancha} from '../repositories/cancha.repository.js';
...
async function getTiposService(){
    try {
        ...
        if(result.length == 0){
            return{
                success: false,
                error: 'No tiene modelos de cancha registrado todavia'
            }        
        }
        return{
            success: true,
...
async function proccesNewModelDataService(data){
    ...
    try {
        ...
        if(!result){
            return {
                success: false,
                error: 'No se pudo crear un usuario, intente nuevamente.'
            }
        }
        return{
            success: true,
...
async function editModelService(modeloId){
    try {
        ...
        if(!result){
            return {
                success: false,
                error: 'No se encontro el modelo'
            }
        }
        return {
            success: true,
...
async function deleteModelService(modelId){
    try {
        ...
        if(!result){
            return {
                success: false,
                error: 'Error al eliminar modelo, intente nuevamente.'
            }
        }
        return{
            success: true,
...
async function postNewCanchaService(canchaData){
    try {   
        ...
        if(!result){
            return {
                success: false,
                error: 'Erro al crear nueva cancha, intente nuevamente'
            }
        }
        return{
            success: true,
...
async function editCanchaService(canchaId){
    try {
        ...
        if(!result){
            return{
                success: false,
                error: 'No se pudo obtener la cancha'
            }
        }
        return {
            success: true,
